import er, {
	clearVfsWriteCache as ir,
	takeCachedVfsWriteBase64 as nr,
	registerJsCallBatch as rr,
	webFsReadBase64 as sr,
	ExtensionSession as tr,
	setLogLevel as Ye,
...
const je = {
	...
	ar = ["trace", "debug", "info", "warn", "error", "none"];
function or(t) {
...
`)[0];
	...
	if (typeof t == "object")
		try {
			...
			return n instanceof TypeError && n.message.includes("circular")
				? "[Circular]"
				: `[Unserializable: ${n instanceof Error ? n.message : String(n)}]`;
		}
	return String(t);
}
function ur(t) {
...
function fr(t, e) {
	...
	const r = [];
	try {
		for (const [n, s] of Object.entries(t)) r.push(`${n}=${Nt(s, e)}`);
	} catch {
...
class lt {
	...
	timer(e, r, n = "info") {
		const s = typeof performance < "u" && performance.now,
			i = s ? performance.now() : Date.now();
		return (c) => {
			try {
				const a = s ? performance.now() : Date.now(),
					h = Math.round(a - i),
					d = {
						...r,
						...c,
...
function he(t) {
	return t == null
		? {}
		: t instanceof Map
			? Object.fromEntries([...t.entries()].map(([e, r]) => [e, he(r)]))
			: Array.isArray(t)
				? t.map(he)
				: t;
}
	...
	(t.assertNever = r),
		...
		(t.getValidEnumValues = (s) => {
			const i = t.objectKeys(s).filter((a) => typeof s[s[a]] != "number"),
				c = {};
			for (const a of i) c[a] = s[a];
			return t.objectValues(c);
		}),
		(t.objectValues = (s) => t.objectKeys(s).map((i) => s[i])),
		(t.objectKeys =
			typeof Object.keys == "function"
				? (s) => Object.keys(s)
				: (s) => {
						const i = [];
						for (const c in s) Object.hasOwn(s, c) && i.push(c);
						return i;
					}),
		(t.find = (s, i) => {
		...
		const r = e || ((i) => i.message),
			...
			s = (i) => {
				for (const c of i.issues)
					...
					else if (c.path.length === 0) n._errors.push(r(c));
					else {
						let a = n,
							h = 0;
						for (; h < c.path.length; ) {
							const d = c.path[h];
							h === c.path.length - 1
								? ((a[d] = a[d] || { _errors: [] }), a[d]._errors.push(r(c)))
								: (a[d] = a[d] || { _errors: [] }),
								(a = a[d]),
								h++;
						}
...
const Xe = (t) => {
	...
	const h = n
		...
		.slice()
		.reverse();
	for (const d of h) a = d(c, { data: e, defaultError: a }).message;
	return {
		...s,
		path: i,
		message: a,
...
class $ {
	...
	static mergeObjectSync(e, r) {
		...
		for (const s of r) {
			...
			if (i.status === "aborted" || c.status === "aborted") return v;
...
const v = Object.freeze({
	...
	_e = (t) => ({ status: "dirty", value: t }),
	L = (t) => ({ status: "valid", value: t }),
	vt = (t) => t.status === "aborted",
	kt = (t) => t.status === "dirty",
	pe = (t) => t.status === "valid",
	De = (t) => typeof Promise < "u" && t instanceof Promise;
var y;
((t) => {
...
const wt = (t, e) => {
	if (pe(e)) return { success: !0, data: e.value };
		...
		: {
				errorMap: (c, a) => {
					...
					return c.code === "invalid_enum_value"
						...
						: typeof a.data > "u"
							? { message: h ?? n ?? a.defaultError }
							: c.code !== "invalid_type"
								? { message: a.defaultError }
								: { message: h ?? r ?? a.defaultError };
				},
				description: s,
			};
}
class S {
	...
	_parseSync(e) {
		...
		if (De(r)) throw new Error("Synchronous parse encountered promise.");
		return r;
	}
	_parseAsync(e) {
		const r = this._parse(e);
	...
	async safeParseAsync(e, r) {
		const n = {
			...
			i = await (De(s) ? s : Promise.resolve(s));
		return wt(n, i);
	}
	refine(e, r) {
		const n = (s) =>
...
const mr = /^c[^\s-]{8,}$/i,
	gr = /^[0-9a-z]+$/,
	yr = /^[0-9A-HJKMNP-TV-Z]{26}$/i,
	br =
		/^[0-9a-fA-F]{8}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{12}$/i,
	_r = /^[a-z0-9_-]{21}$/i,
	vr = /^[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]*$/,
	kr =
...
const xr =
	...
	Ar = /^([0-9a-zA-Z-_]{4})*(([0-9a-zA-Z-_]{2}(==)?)|([0-9a-zA-Z-_]{3}(=)?))?$/,
	Mt =
...
function Lr(t, e) {
	const r = (t.toString().split(".")[1] || "").length,
		...
		c = Number.parseInt(e.toFixed(s).replace(".", ""));
	return (i % c) / 10 ** s;
}
class oe extends S {
	constructor() {
...
class W extends S {
	_parse(e) {
		...
		if (
			...
			return Promise.all(
				[...r.data].map((c, a) => s.type._parseAsync(new B(r, c, r.path, a))),
			).then((c) => $.mergeArray(n, c));
		const i = [...r.data].map((c, a) =>
			s.type._parseSync(new B(r, c, r.path, a)),
		);
...
function ue(t) {
	if (t instanceof A) {
		...
		return t instanceof W
			...
			: t instanceof V
				? V.create(ue(t.unwrap()))
				: t instanceof ne
					? ne.create(ue(t.unwrap()))
					: t instanceof z
						? z.create(t.items.map((e) => ue(e)))
						: t;
}
class A extends S {
	constructor() {
	...
	_parse(e) {
		...
		const { status: n, ctx: s } = this._processInputParams(e),
			...
			a = [];
		if (
			!(this._def.catchall instanceof te && this._def.unknownKeys === "strip")
		)
			for (const d in s.data) c.includes(d) || a.push(d);
		const h = [];
		for (const d of c) {
			const m = i[d],
				R = s.data[d];
							...
							errorMap: (r, n) => {
								...
								const s =
									((c = (i = this._def).errorMap) == null
										? void 0
										: c.call(i, r, n).message) ?? n.defaultError;
	...
	omit(e) {
		const r = {};
		for (const n of I.objectKeys(this.shape)) e[n] || (r[n] = this.shape[n]);
		return new A({
			...this._def,
			shape: () => r,
		});
	...
	required(e) {
		...
		for (const n of I.objectKeys(this.shape))
			if (e && !e[n]) r[n] = this.shape[n];
				...
				let i = this.shape[n];
				for (; i instanceof V; ) i = i._def.innerType;
				r[n] = i;
			}
		return new A({
			...this._def,
	...
	_parse(e) {
			...
			).then(s);
		{
			...
			for (const h of n) {
				...
				m.status === "dirty" && !i && (i = { result: m, ctx: d }),
							...
							: t instanceof We
								...
								: t instanceof Se
									...
									: t instanceof V
										...
										: t instanceof ne
											? [null, ...J(t.unwrap())]
											: t instanceof ut || t instanceof Me
												? J(t.unwrap())
												: t instanceof Ne
													? J(t._def.innerType)
													: [];
...
function rt(t, e) {
	...
	if (r === g.object && n === g.object) {
		const s = I.objectKeys(e),
			i = I.objectKeys(t).filter((a) => s.indexOf(a) !== -1),
			c = { ...t, ...e };
		for (const a of i) {
			const h = rt(t[a], e[a]);
...
class Ee extends S {
	_parse(e) {
		const { status: r, ctx: n } = this._processInputParams(e),
			s = (i, c) => {
				if (vt(i) || vt(c)) return v;
				const a = rt(i.value, c.value);
				return a.valid
					? ((kt(i) || kt(c)) && r.dirty(), { status: r.value, value: a.data })
					: (p(n, {
...
class z extends S {
	_parse(e) {
		...
		const i = [...n.data]
			...
			.filter((c) => !!c);
		return n.common.async
			? Promise.all(i).then((c) => $.mergeArray(r, c))
			: $.mergeArray(r, i);
	}
	get items() {
		...
		if (n.common.async) {
			...
			return Promise.resolve().then(async () => {
				for (const h of c) {
					const d = await h.key,
						m = await h.value;
					if (d.status === "aborted" || m.status === "aborted") return v;
					(d.status === "dirty" || m.status === "dirty") && r.dirty(),
						a.set(d.value, m.value);
				}
...
class me extends S {
	_parse(e) {
		...
		const a = [...n.data.values()].map((h, d) =>
			i._parse(new B(n, h, n.path, d)),
		);
	...
	_parse(e) {
		...
		if (this._def.returns instanceof ge) {
			...
			return L(async function (...h) {
				const d = new j([]),
					m = await a._def.args.parseAsync(h, i).catch((Q) => {
			...
			return L(function (...h) {
				...
				if (!d.success) throw new j([n(h, d.error)]);
				const m = Reflect.apply(c, this, d.data),
					R = a._def.returns.safeParse(m, i);
				if (!R.success) throw new j([s(m, R.error)]);
				return R.data;
			});
		}
	}
...
class st extends S {
	_parse(e) {
		const r = I.getValidEnumValues(this._def.values),
				...
				return this._def.schema
					...
					.then((a) =>
						a.status === "aborted"
							? v
							: (a.status === "dirty" && r.dirty(),
								c(a.value).then(() => ({ status: r.value, value: a.value }))),
					);
		}
		if (s.type === "transform")
			if (n.common.async === !1) {
				...
				return this._def.schema
					...
					.then((c) =>
						pe(c)
							? Promise.resolve(s.transform(c.value, i)).then((a) => ({
									status: r.value,
									value: a,
								}))
							: v,
...
class Me extends S {
	_parse(e) {
		const r = this._def.innerType._parse(e),
			n = (s) => (pe(s) && (s.value = Object.freeze(s.value)), s);
...
function M(t, e = 0, r = 2) {
	...
	if (t instanceof A) {
		const n = t.shape,
			s = Object.keys(n).filter((c) => !c.startsWith("__"));
		return s.length === 0
			? "{ }"
			: e >= r - 1
				? "{ ... }"
				: `{ ${s
						.map((c) => {
							const a = n[c],
								h = a instanceof V,
								d = M(h ? a.unwrap() : a, e + 1, r);
							return `${c}${h ? "?" : ""}: ${d}`;
						})
						.join(", ")} }`;
	}
	if (t instanceof Ie) return t.options.map((n) => M(n, e, r)).join(" or ");
	...
	if (t instanceof Re) {
		const n = M(t._def.valueType, e + 1, r);
						...
						: t instanceof tt
							...
							: t instanceof We
								...
								: t instanceof Y
									? M(t.innerType(), e, r)
									: t instanceof Oe
										? M(t.removeDefault(), e, r)
										: t instanceof ne
											? `${M(t.unwrap(), e, r)} | null`
											: t instanceof Ae
												? "lazy"
												: t instanceof ge
													? `Promise<${M(t.unwrap(), e + 1, r)}>`
													: t instanceof ke
														? "function"
														: t instanceof xe
															? "Date"
															: t instanceof nt
																...
																: t instanceof me
																	? "Set"
																	: t instanceof Ee
																		? `${M(t._def.left, e, r)} & ${M(t._def.right, e, r)}`
																		: t instanceof ze
																			? t.options
																					.map((n) => M(n, e, r))
																					.join(" or ")
...
function Vr(t, e, r, n) {
	const s = r.filter((a) => a.path.length === 0),
		i = r.filter((a) => a.path.length > 0);
	if (s.length > 0 && i.length === 0) {
		const a = s.some((d) => d.code === "custom"),
			h = s.some(
				(d) =>
					d.code !== "invalid_type" &&
	...
	const c = r.map(
		(a) =>
			`at '${a.path.length > 0 ? a.path.join(".") : "root"}': ${a.message}`,
	);
...
function Yr(t) {
	for (const e of t) Br(e.action, Jr(e.action, e.owner));
}
...
K((t) => (Array.isArray(t) ? { keys: t } : t), Qr);
l({});
const Xr = l({
	keys: E(o()).describe("Array of storage keys to delete"),
});
K((t) => (Array.isArray(t) ? { keys: t } : t), Xr);
l({});
l({});
x([
	Dr([x([l({ text: o() }), o()])]),
...
K((t) => (Array.isArray(t) ? { fields: t } : t), nn);
x([f(), E(l({}).passthrough()), l({}).passthrough()]);
l({});
l({
...
const an = l({
	...
	on = l({
		filter: an.optional().describe("Semantic filter criteria"),
		max_nodes: f()
			.optional()
...
l({
	...
	bodyEncoding: Pe(["text", "base64", "handle"]).describe(
...
const Ft = l({
	...
	Ut = l({
		...
		tabs: hn.optional().describe("Array of tabs in the window"),
...
function En(t) {
	...
	for (let r = 0; r < t.length; r += It) {
		...
		for (let s = 0; s < n.length; s++) e += String.fromCharCode(n[s]);
	}
	return btoa(e);
}
function Rn(t) {
...
function zt(t, e, r) {
	if (!Rn(t) || typeof e != "object" || e === null || !("bodyEncoding" in e))
		return e;
	const n = e;
	if (n.bodyEncoding !== "base64" || !n.body) return e;
	const s = In(n.body),
		i = Tn(r, s, {
...
async function An(t, e, r, n) {
	...
	if (!i.success)
		return Je(
			((h = i.error.issues[0]) == null ? void 0 : h.message) ??
				"Invalid setFiles params",
		);
	const c = i.data,
		a = [];
	for (const d of c.files) {
		...
		if (d.handle) {
			...
			if (!m) return Je(`Unknown or expired handle: ${d.handle}`);
			a.push({
				kind: "bytes",
...
const qt = /* @__PURE__ */ new Set(["page_set_files", "tab_set_files"]),
	Ht = /* @__PURE__ */ new Set(["page_fetch", "tab_fetch"]);
let ie = null,
	at = !1;
...
function Gt(t, e, r) {
	e === "main-thread" || e === "content-script"
		? Be(r, { type: "relayCancel", id: t, owner: e })
		: Be(r, { type: "registryCallCancel", id: t });
}
function At(t, e) {
	for (const [r, n] of fe)
...
function Jt(t) {
	Z &&
		(C.error("runCell_worker_failure", {
			runId: Z.runId,
			callId: Z.id,
			error: t,
		}),
...
self.addEventListener("error", (t) => {
	const e =
		t.message ||
		(t.error instanceof Error ? t.error.message : "Worker uncaught error");
	C.error("worker_uncaught_error", { error: e }), Jt(e);
});
self.addEventListener("unhandledrejection", (t) => {
	const e = t.reason,
		r = e instanceof Error ? e.message : String(e ?? "Unhandled rejection");
...
function N(t, e, r) {
	Pn(t, async (n, s) => {
		...
		if (!i.success) {
			const c = Vr(t, e, i.error.issues, n),
				a = new Error(c);
...
function jn(t, e) {
	...
	ot.set(t, e),
		e.addEventListener("message", async (n) => {
			...
			if (
				s !== null &&
				(s.type === "asyncRelayResult" || s.type === "registryCallResult") &&
				typeof s.id == "string"
			) {
				ct(s.id, s.result);
				return;
			...
			if (
				...
				Le.delete(c),
					Be(e, { type: "registryCallResult", id: c, result: d }) ||
						ct(c, {
							ok: !1,
							error: {
...
function ct(t, e) {
	...
	return r
		? (r.settle(e), !0)
		: (C.warn("asyncRelayResult_no_pending_relay", { id: t }), !1);
}
function Dn(t) {
	if (t === "main-thread" || t === "content-script") return self;
	const e = ot.get(t);
	return e || null;
}
function Yt(t) {
	const { owner: e, action: r, tabPolicy: n, resolveTimeoutMs: s } = t,
		i = t.timeoutMs ?? ve;
	return (c, a) => {
		...
		return new Promise((d, m) => {
			...
			const de = (G) => {
					Q ||
						((Q = !0),
						clearTimeout(mt),
						a != null && a.signal && a.signal.removeEventListener("abort", He),
						fe.delete(P),
						G());
				},
				pt = () => {
			...
			const gt = a == null ? void 0 : a.runId,
				yt = a == null ? void 0 : a.callId;
			Be(R, {
				type:
					e === "main-thread" || e === "content-script"
						? "asyncRelay"
				...
				runId: gt,
			}) || de(() => m(new Error(`Failed to post relay for action: ${r}`)));
		});
	};
}
const Zn = "worker",
...
const Bn = 6e4;
function Qt(t, e) {
	if (qt.has(t)) return Bn;
	const r = Vn[t];
	if (!r) return ve;
	let n = Fn(e, r);
	return (
		n === null && Kt.has(t) && (n = ve),
		n === null ? ve : Math.max(ve, Un(t, n))
	);
}
async function Xt(t, e, r) {
	if (!qt.has(t)) return { ok: !0, params: e };
	if (!ie)
		return {
			ok: !1,
			error: { message: "Session not initialized", code: "E_INTERNAL" },
		};
	const n = await An(t, e, r, async (s) => {
		const i = nr(s);
...
function Jn(t, e) {
	...
	return (async () => {
		var h, d;
		const s = await Xt(r, t, e == null ? void 0 : e.runId);
		if (!s.ok) return s;
		...
		return Ht.has(r) && typeof a == "object" && a !== null && "ok" in a && a.ok
			? {
					ok: !0,
					value: zt(n, a.value, e == null ? void 0 : e.runId),
				}
			: a;
	})();
}
function zn(t) {
	if (t.owner === Zn) {
		...
		return async (r, n) => {
			...
			r = he(r);
			const s =
				(n == null ? void 0 : n.signal) ??
				(n != null && n.runId
					? (i = F.get(n.runId)) == null
						? void 0
						: i.signal
					: void 0);
			try {
		...
		return async (r, n) => {
			var c;
			const s = he(r),
				i = await Xt(t.action, s, n == null ? void 0 : n.runId);
			if (!i.ok) return i;
			try {
				...
				return typeof a == "object" &&
					...
					? {
							ok: !0,
							value: zt(s, a.value, n == null ? void 0 : n.runId),
						}
					: a;
			} catch (a) {
...
function qn(t) {
	N("exists", X, (e) => t.fsExists(e)),
		N("stat", X, (e) => t.fsStat(e)),
		N("read", X, (e) => t.fsRead(e)),
		N("readText", X, (e) => t.fsReadText(e)),
		N("readBase64", X, (e) => t.fsReadBase64(e)),
		N("list", X, (e) => t.fsList(e)),
		N("mkdir", X, (e) => t.fsMkdir(e)),
		N("delete", X, (e) => t.fsDelete(e)),
		N("copy", xt, (e) => t.fsCopy(e)),
		N("move", xt, (e) => t.fsMove(e)),
		N("write", le, (e) => t.fsWrite(e)),
		N("writeText", le, (e) => t.fsWriteText(e)),
		N("writeBase64", le, async (e) => {
			const r = e;
			return Cn(r.path, r.data), t.fsWriteBase64(r);
		}),
		N("append", le, (e) => t.fsAppend(e)),
		N("appendText", le, (e) => t.fsAppendText(e)),
		N("appendBase64", le, (e) => t.fsAppendBase64(e)),
		N("readRange", cn, (e) => t.fsReadRange(e)),
		N("update", dn, (e) => t.fsUpdate(e)),
		N("hash", ln, (e) => t.fsHash(e));
}
...
self.onmessage = async (t) => {
	...
	if (e.type === "init") {
		try {
			...
			C.error("worker_init_failed", { error: s }),
				self.postMessage({
					type: "error",
	...
	await Mn(async () => {
		switch (e.type) {
			case "runCell": {
				...
				n && F.set(n, s),
					(Z = { id: e.id, runId: n }),
					C.trace("runCell_start", {
						runId: n,
						callId: e.id,
						codeLen: e.code.length,
					});
				try {
					const i = await r.runCellAsync(e.code, e.stdin || "", n || "");
					...
					const c = i instanceof Error ? i.message : String(i),
						a = i instanceof Error ? i.name : void 0,
						h = i instanceof Error ? i.stack : void 0;
					let d;
					if (h) {
					...
					(Z == null ? void 0 : Z.id) === e.id && (Z = null),
						n && (F.delete(n), Sn(n));
				}
				break;
			}
			...
			case "stop": {
				...
				F.clear(),
					St(),
					At("E_STOPPED", "Worker stopped"),
					self.postMessage({ type: "result", id: e.id, data: { ok: !0 } });
				break;
			}
			case "setFuelLimit": {
				try {
					r.set_fuel_limit(e.limit),
						e.id &&
							self.postMessage({
								type: "result",
								id: e.id,
