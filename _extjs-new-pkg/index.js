...
var Zt = (e, r, n) =>
	r in e
		? Vt(e, r, { enumerable: !0, configurable: !0, writable: !0, value: n })
		: (e[r] = n);
var M = (e, r, n) => Zt(e, typeof r != "symbol" ? r + "" : r, n);

import {
	init as Qt,
	formatSnapshot as Ze,
	collectDocument as zt,
...
`)[0];
	...
	if (typeof e == "object")
		try {
			...
			return a instanceof TypeError && a.message.includes("circular")
				? "[Circular]"
				: `[Unserializable: ${a instanceof Error ? a.message : String(a)}]`;
		}
	return String(e);
}
function Yt(e) {
...
function er(e, r) {
	...
	const n = [];
	try {
		for (const [a, o] of Object.entries(e)) n.push(`${a}=${Qe(o, r)}`);
	} catch {
...
class Ae {
	...
	timer(r, n, a = "info") {
		const o = typeof performance < "u" && performance.now,
			i = o ? performance.now() : Date.now();
		return (s) => {
			try {
				const c = o ? performance.now() : Date.now(),
					l = Math.round(c - i),
					u = {
						...n,
						...s,
...
function se(e) {
	return e == null
		? {}
		: e instanceof Map
			? Object.fromEntries([...e.entries()].map(([r, n]) => [r, se(n)]))
			: Array.isArray(e)
				? e.map(se)
				: e;
}
...
function fe(e, r) {
	const n = r || "unknown url",
		a = {
			message:
				e !== void 0
					? `Content script is not connected on tab ${e} (${n}).`
					: `Content script is not connected on this tab (${n}).`,
...
function or(e, r) {
	var s, c;
	const n = e ? "refId" : r != null && r.label ? "label" : null,
		a = e || (r == null ? void 0 : r.label) || "";
	let o = `Element not found${n ? ` by ${n} "${a}"` : ""}`;
	if (r != null && r.label && (s = r.candidates) != null && s.length) {
		const l = r.candidates
			.map((u) => u.name || u.refId)
			.filter(Boolean)
			.slice(0, 5);
		l.length > 0
			? (o += `. Candidates: ${l.join(", ")}`)
	...
	return (
		(c = r == null ? void 0 : r.candidates) != null &&
			c.length &&
			(i.details = { ...i.details, candidates: r.candidates }),
		i
	);
}
function ir(e) {
	return (
		...
		e.includes("Timeout waiting for content-script ping") ||
		e.includes("content script not available") ||
		e.includes("message port closed before a response was received")
	);
}
function Se(e) {
	...
	throw (
		...
		e.hint && (r.hint = e.hint),
		e.recovery && (r.recovery = e.recovery),
		e.details && (r.details = e.details),
		r)
	);
}
function sr(e, r) {
...
function cr(e) {
	...
	const r = e.name !== "Error" ? e.name : void 0,
		n = e.stack;
	let a;
	if (n) {
...
function Ye(e, r) {
	if (
		...
		const c = e;
		return c.hint || c.recovery
			? c
			: c.code === "E_CONTENT_SCRIPT"
				? fe(r == null ? void 0 : r.tabId, r == null ? void 0 : r.url)
				: c;
	}
	const n = (e instanceof Error ? e.message : String(e)) || "",
		{ name: a, stack: o, line: i } = cr(e);
	if (ir(n))
...
function Z(e) {
	return e instanceof t.ZodEffects
		? Z(e.innerType())
		: e instanceof t.ZodDefault
			? Z(e.removeDefault())
			: e instanceof t.ZodPipeline
				? Z(e._def.in)
				: e instanceof t.ZodOptional ||
						e instanceof t.ZodNullable ||
						e instanceof t.ZodBranded ||
						e instanceof t.ZodReadonly
					? Z(e.unwrap())
					: e instanceof t.ZodCatch
						? Z(e.removeCatch())
						: e;
}
function T(e, r = 0, n = 2) {
	...
	if (e instanceof t.ZodObject) {
		const a = e.shape,
			o = Object.keys(a).filter((s) => !s.startsWith("__"));
		return o.length === 0
			? "{ }"
			: r >= n - 1
				? "{ ... }"
				: `{ ${o
						.map((s) => {
							const c = a[s],
								l = c instanceof t.ZodOptional,
								u = T(l ? c.unwrap() : c, r + 1, n);
							return `${s}${l ? "?" : ""}: ${u}`;
						})
						.join(", ")} }`;
	}
	...
	if (e instanceof t.ZodRecord) {
		const a = T(e._def.valueType, r + 1, n);
		return a === "unknown" || a === "any"
			? "{ [key: string]: unknown }"
			: `{ [key: string]: ${a} }`;
	}
	return e instanceof t.ZodOptional
		? `${T(e.unwrap(), r, n)}?`
		: e instanceof t.ZodLiteral
			? JSON.stringify(e.value)
			: e instanceof t.ZodEnum
				? e.options.map((a) => `"${a}"`).join(" | ")
				: e instanceof t.ZodAny
					? "any"
					: e instanceof t.ZodUnknown
						? "unknown"
						: e instanceof t.ZodVoid
							? "void"
							: e instanceof t.ZodUndefined
								? "undefined"
								: e instanceof t.ZodEffects
									? T(e.innerType(), r, n)
									: e instanceof t.ZodDefault
										? T(e.removeDefault(), r, n)
										: e instanceof t.ZodNullable
											? `${T(e.unwrap(), r, n)} | null`
											: e instanceof t.ZodLazy
												? "lazy"
												: e instanceof t.ZodPromise
													? `Promise<${T(e.unwrap(), r + 1, n)}>`
													: e instanceof t.ZodFunction
														? "function"
														: e instanceof t.ZodDate
															? "Date"
															: e instanceof t.ZodMap
																? "Map"
																: e instanceof t.ZodSet
																	? "Set"
																	: e instanceof t.ZodIntersection
																		? `${T(e._def.left, r, n)} & ${T(e._def.right, r, n)}`
																		: e instanceof t.ZodDiscriminatedUnion
																			? e.options
																					.map((a) => T(a, r, n))
																					.join(" or ")
																			: e instanceof t.ZodBranded
																				? T(e.unwrap(), r, n)
																				: e instanceof t.ZodNaN
																					? "NaN"
																					: e instanceof t.ZodCatch
																						? T(e.removeCatch(), r, n)
																						: e instanceof t.ZodPipeline
																							? T(e._def.in, r, n)
																							: e instanceof t.ZodReadonly
																								? `readonly ${T(e.unwrap(), r, n)}`
																								: "unknown";
}
function lr(e) {
	const r = Z(e);
	if (!(r instanceof t.ZodObject)) return [];
	const n = r.shape;
	return Object.keys(n)
		.filter((o) => !o.startsWith("__"))
		.map((o) => {
			const i = n[o];
			let s = !1,
				c = !1,
			...
			const u = T(l, 1, 3) + (c ? " | null" : ""),
				m = i.description ?? l.description ?? "";
			return {
				name: o,
				type: u,
				required: !s,
...
function ur(e, r, n, a) {
	const o = n.filter((c) => c.path.length === 0),
		i = n.filter((c) => c.path.length > 0);
	if (o.length > 0 && i.length === 0) {
		const c = o.some((u) => u.code === "custom"),
			l = o.some(
				(u) =>
					u.code !== "invalid_type" &&
					u.code !== "invalid_literal" &&
					u.code !== "invalid_union",
			);
		if (!c && !l) {
			const u = T(r),
				m = dr(a);
	...
	const s = n.map(
		(c) =>
			`at '${c.path.length > 0 ? c.path.join(".") : "root"}': ${c.message}`,
	);
...
async function mr(e, r, n, a, o) {
	...
	if (!i.success)
		return {
			ok: !1,
			error: {
				message: ur(o, e, i.error.issues, a),
				code: "E_INVALID_PARAMS",
				category: "validation",
			},
		};
	try {
		const s = await n(i.data),
			c = r.safeParse(s);
		if (!c.success) {
...
const ve = h.child("tool-registry"),
	De = /* @__PURE__ */ new Map(),
	pe = /* @__PURE__ */ new Map();
let tt = !1,
	be = null;
...
function p(e) {
	...
	const o = pr(e.action, e.owner),
		i = {
			...e,
			owner: o,
		};
	if ((pe.set(e.action, i), o !== "main-thread")) return;
	const s = {
		action: e.action,
...
function br() {
	...
	for (const n of e)
		n.owner === "main-thread"
			? Oe(n.action) || r.push(`${n.action} (main-thread: no tool handler)`)
			: n.owner === "content-script"
				? Je(n.action) ||
					r.push(
...
function nt() {
	...
	for (const [s, c] of pe) {
		...
		const l =
				c.paramTypes && c.paramTypes.length > 0 ? c.paramTypes : lr(c.params),
			u = {
...
async function C(e, r, n, a, o) {
	...
	return s.ok
		? (ve.debug("dispatch_done", {
				action: e,
				ok: !0,
				resultType: typeof s.value,
			}),
			s)
		: (ve.warn("dispatch_error", {
				action: e,
...
const at = ({ tabId: e }) => {
	...
	ot = (e, r) => {
		...
		(a = n == null ? void 0 : n.runtime) != null &&
			a.id &&
			r.status === "complete" &&
			((F = e), n.tabs.sendMessage(e, { action: "ping" }).catch(() => {}));
	};
function hr(e) {
	var r;
	ge ||
		((r = e == null ? void 0 : e.runtime) != null &&
			r.id &&
			((ge = !0),
			e.tabs.onActivated.addListener(at),
			e.tabs.onUpdated.addListener(ot),
			e.tabs
...
function fr() {
	...
	!((r = e == null ? void 0 : e.runtime) != null && r.id) ||
		!ge ||
		(e.tabs.onActivated.removeListener(at),
		e.tabs.onUpdated.removeListener(ot),
		(ge = !1));
}
async function it() {
	var n;
	...
	if (!((n = r == null ? void 0 : r.runtime) != null && n.id))
		return (
			e.warn("resolveActiveTabId_result", {
				tabId: null,
				reason: "no_extension",
			}),
			null
	...
	try {
		...
		if (o && typeof o.id == "number")
			return (
				(F = o.id), e.debug("resolveActiveTabId_result", { tabId: o.id }), o.id
			);
	} catch {}
	return (
		e.warn("resolveActiveTabId_result", { tabId: null, reason: "not_found" }),
		null
	);
}
function gr(e) {
...
function yr(e, r) {
	const n = r.tabId ?? r.tab_id,
		a = gr(n);
	if (a !== null) return a;
	if (n != null)
		throw new Error("tabId must be a finite number or safe integer bigint");
	if (e === "required") throw new Error("tabId is required for this action");
	if (F !== null) return F;
...
function I(e, r, n, a) {
	...
	return (
		(o.code = r),
		n && (o.category = n),
		a != null && a.hint && (o.hint = a.hint),
		a != null && a.recovery && (o.recovery = a.recovery),
		a != null && a.details && (o.details = a.details),
		o
	);
}
function _r(e) {
...
async function st() {
	...
	if (typeof chrome < "u" && (e = chrome.runtime) != null && e.id)
		try {
			const o =
				((n = (r = chrome.runtime).getManifest) == null
					? void 0
					: n.call(r).permissions) ?? [];
			let i = [];
			(a = chrome.permissions) != null &&
				a.getAll &&
				(i = (await chrome.permissions.getAll()).permissions ?? []),
				(ie = /* @__PURE__ */ new Set([...o, ...i]));
		} catch {
			ie = null;
		}
	else ie = null;
...
function lt(e) {
	...
	if (r === "system" && e.length >= 2) {
		const n = e[1];
		if (n === "cpu" || n === "memory" || n === "storage") return `system.${n}`;
	}
...
const wr = /* @__PURE__ */ Object.freeze(
		/* @__PURE__ */ Object.defineProperty(
			{
				__proto__: null,
				checkPermission: qe,
...
function bt(e, r, n) {
	...
	if (typeof o != "function")
		throw I(`Chrome method not found: ${n}`, "E_EXTENSION", "extension");
	return o.bind(a);
}
function we(e) {
		...
		handler: async (v, g) => {
			...
			try {
				...
				if (
					...
					q !== null &&
					"code" in q &&
					q.code === "E_INVALID_ARGUMENT_TRANSPORT"
				)
					throw q;
				const B = we(q);
				throw (
					(x.debug("chrome_passthrough_err", {
						action: e,
						error: B.error.message,
					}),
					I(B.error.message, B.error.code, B.error.category))
				);
			}
		},
		paramTypes: c,
...
async function ft(e, r) {
	...
	if (!a)
		return (
			n.debug("handleHostCallAction_result", {
				action: e,
				status: "error",
				reason: "no_handler",
			}),
	...
	try {
		const i = await a(r);
		return (
			n.debug("handleHostCallAction_result", { action: e, status: "ok" }),
			{ ok: !0, value: i }
		);
	} catch (i) {
...
const xr = /* @__PURE__ */ new Map([
	...
	Tr = /* @__PURE__ */ new Map([
		...
		["page_wait_for", (e) => ({ selector: e[0], timeout: e[1] ?? BigInt(_e) })],
...
function S(e) {
	if (!e.ok) throw I(e.error.message, e.error.code, e.error.category);
	return e.value;
}
function Ar(e, r) {
	if (!mt(e)) return r;
...
async function Or(e, r) {
	if (!ht(e.action)) return { response: Dr(e.action) };
	if (e.action.startsWith("host_"))
		return { response: await ft(e.action.slice(5), e.params), handler: "host" };
	const n = mt(e.action) ? Ar(e.action, e.params) : Rr(e.action, e.params);
	return { response: await C(e.action, n, e.call_id, e.runId, r) };
}
function qr(e) {
...
function J(e) {
	...
	if (n) return n;
	if (r === "button" || (r === "input" && e.type === "submit")) return "button";
	if (r === "a") return "link";
	if (r === "input") {
		const a = e.type;
															...
															: r === "form"
																...
																: r === "dialog" || r === "modal"
																	...
																	: r === "figure"
																		...
																		: r === "figcaption"
																			? "caption"
																			: e.getAttribute("onclick") || e.onclick
																				? "button"
																				: "generic";
}
function jr(e) {
...
function Hr(e, r = 60) {
	...
	if (n.length > 0) return n.join(" ").slice(0, r);
...
function Ur(e) {
	let r = e;
	for (; r; ) {
		if (r.hidden || r.getAttribute("aria-hidden") === "true" || r.inert)
			return !0;
...
function Fr(e) {
	...
	return o === "status" || o === "alert"
		? !0
		: ((s = e.textContent) == null ? void 0 : s.trim()) || ""
			? !!(wt.has(r) || jr(e))
			: !1;
}
function kt(e) {
	...
	if (i === "generic" && Fr(e)) {
		...
		if (wt.has(m) || e.childElementCount === 0)
			return (
				((l = e.textContent) == null ? void 0 : l.trim().slice(0, 60)) || ""
			);
	}
	return "";
}
...
async function Ee(e) {
	...
	try {
		...
		const n = {};
		e &&
			(e.max_nodes != null && (n.maxNodes = Number(e.max_nodes)),
			e.interactive_only != null && (n.interactiveOnly = e.interactive_only));
		const a = zt(n),
			o = Ze(a, "compact-text");
		return (
			r.debug("handleDomSnapshot_result", { status: "ok" }),
			{
				ok: !0,
				value: { data: a, text: o },
			}
		...
		return (
			r.debug("handleDomSnapshot_result", {
				status: "error",
				error: a || String(n),
			}),
			{
...
async function Wr(e) {
	...
	try {
		await Et();
		const { snapshot: n, format: a } = e,
			o = Ze(n, a);
		return (
			r.debug("handleDomFormat_result", { status: "ok" }), { ok: !0, value: o }
		);
	} catch (n) {
		const a = n instanceof Error ? n.message : String(n);
		return (
			r.debug("handleDomFormat_result", {
				status: "error",
				error: a || String(n),
			}),
			{
...
function Gr(e) {
	...
	for (let n = 0; n < e.length; n += $e) {
		...
		for (let o = 0; o < a.length; o++) r += String.fromCharCode(a[o]);
	}
	return btoa(r);
}
function Br(e) {
...
function $r(e) {
	return e.startsWith("image/") ||
		...
		? !0
		: e.startsWith("application/vnd.");
}
function Kr(e) {
	for (let r = 0; r < e.length; r++) if (e[r] === 0) return !0;
	return !1;
}
function It(e) {
...
async function Zr(e) {
	...
	try {
		const s = new AbortController(),
			c = setTimeout(() => s.abort(), Number(i) ?? _e),
			l = {
				method: n || "GET",
...
const j = () =>
		t.union([t.bigint(), t.number().finite()]).transform((e) => BigInt(e)),
	zr = t.object({
	...
	en = t.preprocess(
		(e) =>
			e !== null && typeof e == "object" && !Array.isArray(e) && !("items" in e)
				? { items: e }
				: e,
		Yr,
	),
	...
	rn = t.preprocess((e) => (Array.isArray(e) ? { keys: e } : e), tn),
	nn = t.object({}),
	an = t.object({
		keys: t.array(t.string()).describe("Array of storage keys to delete"),
	}),
	on = t.preprocess((e) => (Array.isArray(e) ? { keys: e } : e), an),
	sn = t.object({}),
	cn = t.object({}),
	ln = t.union([
...
const In = R({
	...
	Hn = t.preprocess((e) => (Array.isArray(e) ? { fields: e } : e), jn),
	Un = t.union([
		t.number(),
...
const Kn = t.preprocess(
	...
	Ra = t.object({
		...
		format: t
			.string()
			.optional()
			.describe("Output format (e.g. markdown, html)"),
	}),
	Aa = t
		.object({
			max_nodes: t.number().optional().describe("Maximum nodes to include"),
	...
	Oa = t
		.object({
			role: t
				.union([t.string(), t.array(t.string())])
				.optional()
	...
	Dt = t
		.object({
			filter: Oa.optional().describe("Semantic filter criteria"),
			max_nodes: t
				.number()
				.optional()
...
t.record(t.unknown());
t.record(t.unknown());
t.union([t.number(), t.record(t.unknown())]);
const Ma = t.union([t.number(), t.record(t.unknown())]);
t.record(t.unknown());
t.record(t.unknown());
t.record(t.unknown());
t.union([t.string(), t.record(t.unknown())]);
t.record(t.unknown());
t.record(t.unknown());
t.record(t.unknown());
t.record(t.unknown());
t.record(t.unknown());
t.union([t.string(), t.number(), t.record(t.unknown())]);
t.record(t.unknown());
t.record(t.unknown());
t.record(t.unknown());
t.union([t.number(), t.record(t.unknown())]);
...
const Pa = t
	...
	y = t.object({
		...
		disabled: t
			.boolean()
			.optional()
			.describe("Whether the element is disabled"),
		readOnly: t
			.boolean()
			.optional()
			.describe("Whether the element is read-only"),
...
const Re = t.object({
	...
	O = t
		.object({
			...
			status: t
				.string()
				.optional()
				.describe("Tab status (loading or complete)"),
			...
			incognito: t
				.boolean()
				.optional()
				.describe("Whether the tab is incognito"),
			...
			discarded: t
				.boolean()
				.optional()
				.describe("Whether the tab is discarded"),
			autoDiscardable: t
				.boolean()
				.optional()
	...
	le = t
		.object({
			...
			tabs: ee.optional().describe("Array of tabs in the window"),
			incognito: t
				.boolean()
				.optional()
	...
	Ba = t
		.object({
			...
			children: t
				.array(t.object({ id: t.string() }).passthrough())
				.optional()
...
function oo(e) {
	if (typeof e == "string") return P().safeParse(e).success ? e : void 0;
	const r = W(e),
		n =
			typeof r.refId == "string"
				...
				: typeof r.ref_id == "string"
					? r.ref_id
					: void 0;
	if (!(n && !P().safeParse(n).success)) return n;
}
const io = {
...
function so(e, r = io) {
	...
	if (typeof e == "object" && e !== null) {
		const n = e,
			a = typeof n.message == "string" && n.message ? n.message : r.message,
			o = typeof n.code == "string" && n.code ? n.code : r.code,
			i = typeof n.category == "string" ? n.category : r.category,
			s = typeof n.hint == "string" ? n.hint : r.hint,
			c = Array.isArray(n.recovery) ? n.recovery : r.recovery,
			l =
				typeof n.details == "object" && n.details !== null
					? n.details
					: r.details;
		return {
			message: a,
...
function Ut(e) {
	return e && typeof e == "object" && e.ok === !1
		...
		: {
				ok: !0,
				value: e && typeof e == "object" && "value" in e ? e.value : e,
			};
}
function co(e) {
...
async function Lt(e) {
	...
	try {
		const a = await r.tabs.get(e),
			o = a.url ?? "",
			i = a.title ?? "",
			s = `tab ${e} "${i}" (${o || "unknown url"})`;
	...
	for (; Date.now() < o; ) {
		...
		try {
			const u = await Promise.race([
				...
				m = Ut(u);
			return m.ok
				? (n.debug("pingTabContentScript_success", { tabId: e, result: u }),
					{ ok: !0, value: { ok: !0 } })
				: (n.debug("pingTabContentScript_rejected", {
						tabId: e,
						error: m.error,
					}),
					m);
			...
			if (
				((i = m),
				n.debug("pingTabContentScript_retry", { tabId: e, error: m }),
				m.includes("Could not establish connection") ||
					m.includes("Receiving end does not exist") ||
					m.includes("message port closed before a response was received"))
			) {
				await co(Math.min(Sr, o - Date.now()));
				continue;
			}
			if (m.includes("Timeout waiting for content-script ping")) break;
			return we(u);
		}
	}
...
async function Ft(e, r = 3e4, n) {
	var f;
	X();
	const a = h.child("runner"),
		o = typeof e == "number" ? e : null,
		i = n == null ? void 0 : n.preNavigationUrl,
		s = n == null ? void 0 : n.getNavSawLoading;
	...
	const l = (b, v) => {
		...
		u = (b) => i !== void 0 && typeof b == "string" && b.length > 0 && b !== i,
		m = (n == null ? void 0 : n.loadGraceMs) ?? 5e3;
	try {
		return (
					...
					Ge = (D, ne) => {
						D === o &&
							...
							ne.url &&
								...
								q(ne.url)),
							ne.status === "complete" && B());
					};
				c.tabs.onUpdated.addListener(Ge),
					c.tabs
						.get(o)
						.then((D) => {
							re(),
								D.status === "loading" && (x = !0),
								a.debug("waitForTabLoad_initial_status", {
									tabId: o,
									status: D.status,
									url: D.url,
									runId: n == null ? void 0 : n.runId,
...
p({
	...
	handler: async (e, r) => {
		...
		for (const a of Object.keys(n)) {
			const o = n[a];
			localStorage.setItem(`__csl__:${a}`, o == null ? "null" : String(o));
		}
		return null;
	},
	paramTypes: [
...
p({
	...
	handler: async (e, r) => {
		const n = e.keys;
		for (const a of n) localStorage.removeItem(`__csl__:${String(a)}`);
		return null;
	},
	paramTypes: [
		{
...
p({
	...
	handler: async (e, r) => {
		...
		for (const a of n) localStorage.removeItem(a);
		return null;
	},
	paramTypes: [],
	returnDoc: "null",
...
p({
	...
	owner: "main-thread",
	handler: async (e, r) => (
		await new Promise((n) => setTimeout(n, Number(e.duration))), null
	),
	paramTypes: [
		{
			name: "duration",
...
const U =
	...
	Gt = [
		...
		},
		{
			...
			agentMeta: {
				notes: [U, "Content-script path; same refIds as mutations"],
				tags: ["snapshot", "read"],
		...
		},
		{
			...
			returns: t
				.object({
					...
					headings: t
						.array(t.object({ tag: t.string(), text: t.string() }))
						.optional(),
					links: t
						.array(t.object({ href: t.string().nullable(), text: t.string() }))
						.optional(),
					text: t.string().optional(),
				})
				.passthrough(),
...
class uo {
	...
	start() {
		...
		(this.onBeforeRequest = (o) => {
			...
			r.onBeforeRequest.addListener(this.onBeforeRequest, n),
			r.onCompleted.addListener(this.onCompleted, n),
			r.onErrorOccurred.addListener(this.onErrorOccurred, n),
			oe.debug("networkTracker_started", { tabId: this.tabId });
	}
	get pendingCount() {
	...
	async waitForIdle(r, n) {
		...
		for (; Date.now() < a; ) {
			if (this.inFlight.size === 0) {
				if (
					(await new Promise((s) => setTimeout(s, _t)),
			...
			i - o >= 1e3 &&
				((o = i),
				oe.debug("networkTracker_idle_wait", {
					tabId: this.tabId,
					traceId: n,
	...
	dispose() {
		...
		r &&
			(this.onBeforeRequest &&
				r.onBeforeRequest.removeListener(this.onBeforeRequest),
			this.onCompleted && r.onCompleted.removeListener(this.onCompleted),
			this.onErrorOccurred &&
				r.onErrorOccurred.removeListener(this.onErrorOccurred),
			(this.onBeforeRequest = null),
			(this.onCompleted = null),
			(this.onErrorOccurred = null),
			this.inFlight.clear(),
			oe.debug("networkTracker_disposed", { tabId: this.tabId }));
	}
}
async function te(e) {
	const r = await it();
...
p({
	...
	handler: async (e, r) => {
		...
		return S(await C("chrome_tabs_get", [n])).url ?? "";
	},
	paramTypes: [],
...
p({
	...
	handler: async (e, r) => {
		...
		return S(await C("chrome_tabs_get", [n])).title ?? "";
	},
	paramTypes: [],
...
for (const e of Gt.filter((r) => r.namespace === "page")) Bt(e);
p({
	action: "page_goto",
	namespace: "page",
	name: "goto",
	...
	handler: async (e, r) => {
		var v, g, x, N;
		const n = await te("page.goto()"),
			a = r.runId ?? "?";
		if (
			(h.debug("page_goto_start", {
				traceId: a,
				url: e.url,
		...
		const o = await C("chrome_tabs_get", [n]),
			i = o.ok && o.value ? o.value.url : void 0;
		...
		(g = (v = s == null ? void 0 : s.tabs) == null ? void 0 : v.onUpdated) ==
			null || g.addListener(l);
		try {
			const A = await C("chrome_tabs_update", [n, { url: e.url }]);
			if (!A.ok) return S(A);
			...
			(N = (x = s == null ? void 0 : s.tabs) == null ? void 0 : x.onUpdated) ==
				null || N.removeListener(l);
		}
		...
		if (e.waitUntil === "networkidle") {
			...
			try {
				...
				throw (
					(h.debug("page_goto_network_idle_timeout", {
						traceId: a,
		...
		if (!f.ok) return S(f);
		await new Promise((A) => setTimeout(A, vr));
...
p({
	...
	owner: "main-thread",
	handler: async (e, r) => {
		const n = await te("page.health()"),
			a = S(await C("chrome_tabs_get", [n])),
			o = a.url ?? "",
			i = a.title ?? "",
			s = await Lt(n),
			c = s && !s.ok ? "blocked" : "ok",
			u = (await We(n, yt)).ok ? "connected" : "missing",
			m = c === "ok" && u === "connected",
			f = {
				tabId: n,
				url: o,
...
p({
	...
	owner: "main-thread",
	handler: async (e, r) => (
		await new Promise((n) => setTimeout(n, Number(e.duration))), !0
	),
	paramTypes: [
		{
			name: "duration",
...
p({
	...
	handler: async (e, r) => {
		...
		if (n === null) throw I("page_close requires a tabId", "E_MISSING_PARAM");
...
p({
	...
	handler: async (e, r) => {
		const n = typeof e == "number" ? e : Y(e);
		if (n === null) throw I("page_switch requires a tabId", "E_MISSING_PARAM");
		return S(await C("chrome_tabs_update", [n, { active: !0 }]));
	},
	paramTypes: [
		{
			name: "tabId",
...
p({
	...
	owner: "main-thread",
	handler: async (e, r) => {
		const n = await te("page.active_tab()"),
			a = S(await C("chrome_tabs_get", [n]));
...
function K(e) {
	...
	for (const s of n) {
		...
		if (l && l.toLowerCase().trim() === r) return s;
		const u = s.id;
		if (u) {
			const b = document.querySelector(`label[for='${CSS.escape(u)}']`);
			if (
				b &&
				((a = b.textContent) == null ? void 0 : a.trim().toLowerCase()) === r
			)
				return s;
		}
		...
		if (
			(m &&
				((o = m.textContent) == null ? void 0 : o.trim().toLowerCase()) ===
					r) ||
...
function mo(e) {
	...
	for (const i of n) {
		const s = i.getAttribute("aria-label"),
			c = i.placeholder,
			l = ((o = i.textContent) == null ? void 0 : o.trim()) || "";
		if (![s, c, l].filter(Boolean).some((f) => f.toLowerCase().includes(r)))
			continue;
...
function po(e) {
	const r = document.querySelector(`[data-ref-id='${CSS.escape(e)}']`);
	let n, a;
	r && ((n = r.tagName.toLowerCase()), (a = J(r)));
	const o = Array.from(document.querySelectorAll(je)),
		i = [];
	for (const s of o) {
...
const bo = /* @__PURE__ */ new Map([
	...
	[
		"sidepanel_scroll",
		(e, r) => {
			const n = r.direction ?? "down",
				a = typeof r.amount == "number" ? r.amount : gt;
			return (
				window.scrollBy({
					top: n === "up" ? -a : a,
					behavior: "smooth",
...
function H(e, r) {
	...
	if (!i)
		throw (
			(n.error("dispatchSidepanelEvent_no_handler", { action: e }),
			I(`Unknown sidepanel action: ${e}`, "E_UNKNOWN"))
		);
	return i(a, o);
}
p({
...
p({
	...
	handler: async (e, r) => {
		const n = await Ee(e);
		if (!n.ok) throw I(n.error.message, n.error.code, n.error.category);
		if (n.value && typeof n.value == "object") return n.value.text;
...
p({
	...
	handler: async (e, r) => {
		const n = await it();
		if (n === null) throw new Error("No active tab available");
		const a = S(await C("chrome_tabs_get", [n]));
		return { ...a, tabId: typeof a.id == "number" ? a.id : n };
	},
	paramTypes: [],
...
p({
	...
	handler: async (e, r) => {
		const n = typeof e == "number" ? e : Y(e);
		if (n === null) throw I("tab_activate requires a tabId", "E_MISSING_PARAM");
		return S(await C("chrome_tabs_update", [n, { active: !0 }]));
	},
	paramTypes: [
		{
...
p({
	...
	handler: async (e, r) => {
		const n = typeof e == "number" ? e : Y(e);
		if (n === null) throw I("tab_close requires a tabId", "E_MISSING_PARAM");
		return S(await C("chrome_tabs_remove", [n]));
	},
	paramTypes: [
		{
...
for (const e of Gt.filter((r) => r.namespace === "web.tab")) Bt(e);
p({
	...
	handler: async (e, r) => {
		const n = W(e),
			a = Y(e),
			o = typeof n.timeout == "number" ? n.timeout : 3e4;
...
const k = t.unknown(),
	w = t.union([t.null(), t.undefined(), t.boolean()]),
	Q = w;
function _(e, r, n, a = k, o, i) {
	d(e, "chrome", n, r, a, "ECHROME", "extension", [], o, i);
}
d(
...
async function $t(e, r, n, a) {
	...
	const i = Me(e, Pe(a, e)),
		s = bt(o, r, n),
		c = await Promise.resolve(pt(s, i));
	return await ct(), c;
}
_(
...
p({
	...
	returnType: "boolean",
	handler: async (e) =>
		$t("chrome_permissions_remove", ["permissions"], "remove", e),
	paramTypes: [],
	returnDoc: "boolean",
	errorCode: "ECHROME",
...
p({
	...
	returnType: "boolean",
	handler: async (e) =>
		$t("chrome_permissions_request", ["permissions"], "request", e),
	paramTypes: [],
	returnDoc: "boolean",
	errorCode: "ECHROME",
...
_("chrome_tts_stop", ["tts"], "Stop TTS", Q, "chrome.tts.stop()", "null");
d(
...
function ho(e, r) {
	...
	if (
		...
		const n = r[0];
		if ("options" in n) return [n.id ?? "", n.options];
	}
	return [...r];
}
function L(e, r, n, a, o = [], i) {
	...
	p({
		...
		handler: async (f, b) => {
			...
			const g = ho(e, Pe(f, e));
			return (
				v.debug("alias_dispatch", { action: e, target: r, argCount: g.length }),
				S(await C(r, g))
			);
		},
		paramTypes: o,
...
class Kt {
	...
	static async init() {
		...
		if (
			...
			typeof chrome < "u" && (c = chrome.runtime) != null && c.id)
		) {
			hr(chrome);
	...
	startWorker(r) {
					...
					case "error": {
						const v =
							typeof f.error == "string"
								? f.error
								: ((b = f.error) == null ? void 0 : b.message) ||
									"Worker init error";
						a(new Error(v));
						break;
					}
		...
		const l =
			typeof chrome < "u" && (u = chrome.runtime) != null && u.id
				? chrome.runtime.id
				: void 0;
		return (
			c.postMessage({ type: "init", manifest: r, extensionId: l }),
			h.trace("startWorker_posted_init", {
				extensionId: l ?? null,
			}),
			[o, s]
		);
	...
	handleWorkerMessage(r) {
		...
		switch (
			...
			case "result": {
				...
				s
					? (this.pendingCalls.delete(i),
						h.trace("result", { callId: i, runId: n.runId }),
						s.resolve(n.data))
					: h.trace("result_no_pending", { callId: i, runId: n.runId });
				break;
			}
			case "error": {
				const i = n.id,
					...
					c =
						typeof s == "string"
							? s
							: (s == null ? void 0 : s.message) || "Worker error";
			...
			case "relayCancel": {
				if (!n.id) break;
				h.trace("relayCancel", { id: n.id }),
					(a = this.inFlightRelays.get(n.id)) == null || a.abort();
				break;
			}
			case "asyncRelay": {
				...
				if (typeof i != "object" || i === null || !("action" in i)) {
					h.warn("asyncRelay_invalid_command", { id: n.id }),
						(o = this.worker) == null ||
							o.postMessage({
				...
				const s = String(i.action),
					c = n.owner ?? "main-thread",
					l = n.tabPolicy ?? "active",
					u = n.id,
					m = new AbortController();
				this.inFlightRelays.set(u, m),
					h.trace("asyncRelay", {
						action: s,
						owner: c,
						id: u,
				...
				const f = i;
				(f.runId = n.runId),
					this.executeContextCommand(c, f, l, u, m.signal)
						.then((b) => {
							var v;
							if (
								!(
	...
	executeContextCommand(r, n, a = "active", o, i) {
		return (
			...
			i != null && i.aborted
				...
				: r === "main-thread"
					? ht(n.action)
						? this.withMainThreadTimeout(Mr(n, i), n.action)
						: Promise.resolve({
								ok: !1,
								error: {
					...
					: r === "content-script"
						? this.executeContentScriptCommand(n, a, o, i)
						: Promise.resolve({
								ok: !1,
								error: {
	...
	async executeContentScriptCommand(r, n, a, o) {
		...
		const u = () => {
			(l = !0),
				a &&
					i.tabs
						.sendMessage(c, { type: "registryCallCancel", id: a })
						.catch(() => {});
		};
		...
		const o = this.generateId(),
			...
			s = this.runQueue.then(async () => {
				...
				try {
					...
					throw (
						(h.error("runCell_failed", { runId: i, callId: o, error: l }), c)
					);
				}
			});
		return (
			(this.runQueue = s.then(
				() => {},
				() => {},
			)),
			s
	...
	apiDocs(r = "json") {
		...
		return this.postAndWait({ type: "apiDocs", id: n, format: r }).then((a) =>
			r === "json" ? JSON.parse(a) : a,
		);
	}
	setFuelLimit(r) {
		!this.worker ||
			this.disposed ||
			this.worker.postMessage({ type: "setFuelLimit", limit: r });
	}
	loadLibrary(r) {
		const n = this.generateId();
		...
		return {
			query: async (r, n) => {
				const a = await this.executeContentScriptCommand(
					{
						...
						params: {
							filter: r ?? {},
							max_nodes: n == null ? void 0 : n.maxNodes,
						},
					},
				...
				if (typeof a == "object" && a !== null && "ok" in a && !a.ok) {
					const o = a.error;
					throw new Error(
						(o == null ? void 0 : o.message) ?? "snapshot_query failed",
					);
				}
				return a;
			},
	...
	async stopWith(r) {
		if (!this.disposed) {
			...
			for (const [, n] of this.inFlightRelays) n.abort();
			...
			this.pendingCalls.clear(),
				this.onCleanupComplete &&
					(this.onCleanupComplete(), (this.onCleanupComplete = null));
			try {
				await r;
			} catch (n) {
