...
export function startMockAnthropicServer(options: {
  responses: Array<{ chunks: string[]; delays: number[]; stopReason: "end_turn" | "tool_use" }>;
}): MockAnthropicServer {
  ...
  const server = createServer((req, res) => {
    ...
    if (req.url === "/v1/messages" && req.method === "POST") {
      ...
      req.on("end", () => {
        ...
        const response = options.responses[requestBodies.length - 1] ?? { chunks: [], delays: [], stopReason: "end_turn" };
        res.writeHead(200, {
