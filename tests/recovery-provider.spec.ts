...
test("provider bad stream", async () => {
  ...
  await sidePanel.locator('input[placeholder="Type a task..."]').fill("bad stream");
  await sidePanel.getByRole("button", { name: "Run" }).click();

  await expect(
    sidePanel.locator('[data-testid="agent-status"]'),
  ).toHaveText(/error/, { timeout: 10000 });
...
test("provider weak network recovers after retry", async () => {
  ...
  const server = createServer((req, res) => {
    ...
    if (req.url === "/v1/messages" && req.method === "POST") {
      ...
      req.on("end", () => {
        try { requestBodies.push(JSON.parse(body)); } catch { requestBodies.push(body); }
        requestCount++;
        if (requestCount === 1) {
  ...
  await sidePanel.locator('input[placeholder="Type a task..."]').fill("retry task");
  await sidePanel.getByRole("button", { name: "Run" }).click();

  await expect(sidePanel.locator("text=Recovered")).toBeVisible({ timeout: 15000 });
