...
test("debug 207b extension-js init", async () => {
		...
		responses: [
			{
				chunks: [
					...
					`event: content_block_delta\ndata: ${JSON.stringify({ type: "content_block_delta", index: 1, delta: { type: "input_json_delta", partial_json: '{"code":"console.log(\'checked\')"}' } })}\n\n`,
	...
	// Wait longer for tool execution and second response
	await sidePanel.waitForTimeout(8000);
