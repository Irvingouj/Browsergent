...
Simulated shell: the bash tool runs commands against that same OPFS filesystem. It is not the computer's shell — no host programs and no network. The current directory starts at / and persists across bash calls in this session. Use file_edit for an exact text replacement.
